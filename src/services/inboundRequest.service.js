...
import InboundRequest, {
  inboundRequestSchema,
} from "../models/InboundRequest.js";
...
function parseDateTime(value, fieldName) {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new AppError(
      `${fieldName} is not a valid date-time`,
      400,
      "VALIDATION_ERROR",
    );
  }
...
async function assertContractForInbound(tenantId, contractId, warehouseId) {
  ...
  if (contract.tenantId !== tenantId) {
    throw new AppError(
      "contractId does not belong to this tenant",
      400,
      "VALIDATION_ERROR",
    );
  }
  if (contract.warehouseId !== warehouseId) {
    throw new AppError(
      "contractId does not belong to this warehouse",
      400,
      "VALIDATION_ERROR",
    );
  }
...
function assertExpectedArrivalOnOrAfterContractStart(
  contract,
  expectedArrivalDate,
) {
...
function normalizeCreatePayload(body) {
  ...
  if (data.actualArrivalAt != null) {
    data.actualArrivalAt = parseDateTime(
  ...
  if (data.createdBy != null)
    data.createdBy = parseOptionalUserId(data.createdBy, "createdBy");
  if (data.approvedBy != null)
    data.approvedBy = parseOptionalUserId(data.approvedBy, "approvedBy");
  if (data.receivedBy != null)
    data.receivedBy = parseOptionalUserId(data.receivedBy, "receivedBy");
...
async function createInboundItemsInTransaction(
  client,
  inboundRequestId,
  items,
) {
...
export async function createInboundRequest(body) {
  ...
  assertExpectedArrivalOnOrAfterContractStart(
    contract,
    data.expectedArrivalDate,
  );
  ...
  const additionalPieces = items.reduce(
  ...
  try {
    ...
    await createInboundItemsInTransaction(
      client,
...
export async function updateInboundRequest(inboundRequestId, body) {
  ...
  if (data.status !== undefined && data.status !== existing.status) {
    ...
    if (
      ...
      ["APPROVED", "IN_TRANSIT", "ARRIVED"].includes(existing.status)
    ) {
      await assertNoInboundReceivingActivity(id);
    }
