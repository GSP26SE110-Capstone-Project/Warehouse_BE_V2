...
async function normalizeCreatePayload(body, tenantId, warehouseId) {
  ...
  if (data.contractName != null)
    data.contractName = String(data.contractName).trim();
  ...
  if (data.createdBy != null)
    data.createdBy = parseUuid(data.createdBy, "createdBy");
  if (data.approvedBy != null)
    data.approvedBy = parseUuid(data.approvedBy, "approvedBy");
...
async function applyRentalLinkedEffectiveContractDates(data, rentalRequestId) {
  ...
  const rr = await RentalRequest.findById(
    parseUuid(rentalRequestId, "rentalRequestId"),
  );
  ...
  const approveIso =
    toIsoDateOnly(rr.reviewedAt) ??
    toIsoDateOnly(data.startDate) ??
    startOfDayLocal();
...
async function enrichContractEstimatedTotal(contract, user = null) {
  ...
  if (!hasEstimatedTotalAmount(contract)) {
    try {
      ...
      console.warn(
        "[contract] enrich estimated total failed:",
        err?.message ?? err,
      );
    }
  }
...
function applySignatureWorkflow(existing, data) {
  ...
  if (
    bothSigned &&
    (tenantJustSigning || existing.status === "PENDING_APPROVAL")
  ) {
...
async function assertActiveStorageForTenantSign(contractId) {
  const count = await StorageReservation.count({
    contractId,
    status: "ACTIVE",
  });
  if (count === 0) {
...
export async function updateContract(contractId, body) {
  ...
  if (existing.rentalRequestId) {
    if (data.startDate !== undefined || data.endDate !== undefined) {
      await applyRentalLinkedEffectiveContractDates(
        data,
  ...
  if (
    ...
    try {
      ...
      console.warn(
        "[contract] auto price on PENDING_APPROVAL failed:",
        err?.message ?? err,
      );
    }
  }
  ...
  if (
    ...
    void notifyTenantAdminContractPendingApproval(updated).catch((err) => {
      console.warn(
        "[contract] tenant pending approval notify failed:",
        err?.message ?? err,
      );
    });
  }
